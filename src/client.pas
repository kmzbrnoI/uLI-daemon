    ...
    begin
     ...
     if (uLI.connected) then
      begin
       try
         ...
         on E:Exception do
           Application.MessageBox(PChar('Nelze zapnout napájení sběrnice pro ovladače:'+#13#10+E.Message), 'uLI-daemon', MB_OK OR MB_ICONWARNING);
       end;
      end;

    end else begin
     F_Main.P_Client.Color := clYellow;
     F_Main.P_Client.Hint := 'Připojeno k hJOP serveru, NEAUTORIZOVÁNO : '+parsed[5];
     ...
     if (oldAuth) then
      begin
       ...
       Application.MessageBox(PChar('Nepodařilo se autorizovat uživatele!'+#13#10+parsed[5]), 'Autorizace se nezdařila', MB_OK OR MB_ICONWARNING);
      end;
   ...
   if (hvindex = -1) then
    begin
     ...
     if ((parsed[4] = 'ok') or (parsed[4] = 'total')) then begin
      ...
      Application.MessageBox(PChar('Lokomotivu '+parsed[2]+' se nepodařio autorizovat'+#13#10+parsed[5]), 'Loko neautorizováno', MB_OK OR MB_ICONWARNING)
     end;
