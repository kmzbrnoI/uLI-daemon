...
begin
 ...
 while (friendly_string[start] <> '(') do dec(start);
 Result := StrToIntDef(Copy(friendly_string, start+4, Length(friendly_string)-start-5), 0);
end;

procedure EnumuLIDevices(filter:string; res:TList<Integer>);
var GUID_COM : TGUID;                                                           // GUID COM portu
...
begin
  GUID_COM := StringToGUID('{4d36e978-e325-11ce-bfc1-08002be10318}');           // GUID COM portu
  hDevInfo := SetupDiGetClassDevsW(@GUID_COM, nil, 0, DIGCF_PRESENT);           // ziskame vsechna pripojena zarizeni odpovidajici danemu GUID
  if (dword(hDevInfo) <> INVALID_HANDLE_VALUE) then
  begin
    i:=0;
    repeat
      DeviceInfoData.cbSize := sizeof(TSP_DEVINFO_DATA);
      successful := SetupDiEnumDeviceInfo(hDevInfo, i, @DeviceInfoData);        // vraci data zarizeni na indexu \i v seznamu zarizeni \hDevInfo
      if successful then
      begin
        r := CM_Get_Device_IDW(DeviceInfoData.DevInst, @Buffer, MAX_PATH, 0);   // vraci id zarizeni jako string do bufferu \Buffer
